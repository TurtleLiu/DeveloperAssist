# 软件开发助手DeveloperAssist
## 1. 简介
### 1.1 软件失败的原因
软件开发者长期以来受到软件失败的困扰，2020年有65%的软件项目宣告失败或陷入困境（超出预算、进度落后或质量不达预期），造成的损失超过2,600亿美元。为避免软件失败，我们分析了140个软件项目失败案例，找出导致失败的共性原因：
>1. 软件能力没有帮助用户解决痛点问题，或效果未达到用户预期；
>2. 缺少使软件持续优化的理论方法、应用框架或技术平台，导致无法持续、快速应对用户需求的变化；
>3. 软件的收费模式无法支撑持续升级优化、开发团队获得利润。

### 1.2 软件生存的前提条件
从上述失败原因中，我们推导出如下软件生存的前提条件
>1. 在特定应用场景下，人类认知能力的提升可以产生价值；
>2. 以软件能力的形式弥补人类认知能力缺陷，当前与其他形式在效果、成本上相比具有较大优势；
>3. 软件带来的认知能力提升使软件用户愿意为此支付新的费用，或使软件运营得到降本增效的改进；
>4. 当人类对认知能力提升需求的变化时，软件能力本身或经快速改进能够适应这些需求。
...
>2. 如何开发新的软件，为用户弥补其能力不足？如帮助心理咨询师应对增长的咨询需求。  
...
以下是实际编程中，使用DeveloperAssist进行单元测试的详细步骤和注意事项：
* **步骤一：启动DeveloperAssist**  
确保您已经在IDE中安装最新版的DeveloperAssist插件，并完成登录使用。
* **步骤二：选择需要测试的代码段**  
在您的代码中选择需要进行单元测试的函数或模块，可以是新增的功能、修复的Bug或是经常容易出错的部分。选中的代码会自动添加到提问栏中，在提问栏中输入重构的要求，如**如果要重构，你有什么建议？**，即可自动生成重构代码。
<div align = "center">
    <img src="images/code_refactor_1.png" width="500">  
</div>  
<div align = "center">
    <img src="images/code_refactor_2.png" width="500">  
</div>  

* **注意：添加设计模式有助于代码重构效果的提升**  
您可以在提问当中，描述希望用于重构代码的设计模式，DeveloperAssist在生成重构代码的过程中将参考这些模式。

**1.3.2.x 软件验证和自动测试**  
当前版本支持针对一段函数，生成对应的单元测试代码

以下是实际编程中，使用DeveloperAssist进行单元测试的详细步骤和注意事项：
* **步骤一：启动DeveloperAssist**  
确保您已经在IDE中安装最新版的DeveloperAssist插件，并完成登录使用。
* **步骤二：选择需要测试的代码段**  
在您的代码中选择需要进行单元测试的函数或模块，可以是新增的功能、修复的Bug或是经常容易出错的部分。选中的代码会自动添加到提问栏中，在提问栏中输入**test**，即可自动生成单元测试代码。
<div align = "center">
    <img src="images/function_test_1.png" width="500">  
</div>  

* **注意：添加注释有助于单元测试效果的提升**  
您可以在选定代码段的上方，添加简单清晰的注释，描述函数功能、预期输出、涉及的边界条件或特殊情况等，DeveloperAssist在生成单元测试的过程中将参考这些注释。

#### 1.3.3 定制微调
点击 **“train”** 按钮，可以对DeveloperAssist的模型进行微调，得到一个适应于用户个性化需求的定制模型。
```bash
... 未完待续 |•'-'•) ✧...
```

## 2. 重现实验步骤和方法
本章介绍微调预训练模型得到DeveloperAssist的**提问引擎**的过程，帮助读者重现我们的实验。如Fig.3所示，实验分为三个步骤，（1）准备阶段：搭建基本实验环境、选择基础模型、基础模型的量化加载。（2）微调阶段：数据集的选择和预处理、模型的训练过程
<div align = "center">
    <img src="images/exp_process.png" width="500">
...
2. 基座模型需要具有较快的推理速度。
3. 基座模型需要适应问答生成任务，因为DeveloperAssist的核心功能是问答。
4. 基座模型需要具有良好的文本理解能力，因为我们需要在此基础上实现对开发者提交文档的分析。
5. 基座模型需要具有良好的常识认知能力，因为在此基础上能够更好使用专业知识向软件开发者提问。
...
### 2.2 数据集和预处理
#### 2.2.1 数据集选择：
根据DeveloperAssist提问引擎的应用场景，微调基础模型的数据格式为 **（开发者提交物，任务类型，提问）** 的三元组，下面给出一个数据样例：
>**开发者提交物**：作为停车用户，我希望能够获得停车的引导；作为停车场运营商，我希望能够检测空闲的车位并合理调度用户停车。  
>**任务类型**：需求分析  
...
## 3. 项目总结
在本项目中，我们为解决软件开发中常见的两个痛点：**为现有软件创造新价值**、**为弥补用户能力不足开发新软件**，提出了一个解决方案：**微调基础模型，构建提问引擎引导软件开发**。

我们实现了将47B参数量的基座模型在两个27GB的GPU上微调的过程，并完成了在Benchmark上的测试，测试结果表明我们的提问引擎模型在多个测试指标上超过了当前最先进的基础模型，证明了我们的模型在下游任务上的有效性。

我们还以我们的模型为核心构建出来了一个完整的应用，实现了本文档开头所描述的功能，初步具备了在实际应用的能力。用户试用的结果显示我们的应用具备了开发有价值软件的能力，但模型给出内容的专业性还需要进一步提升。

在本项目中，我们还从专业心理咨询案例中构建出来一个数据集，格式为  **（开发者提交物，任务类型，提问）** ，可以用作后续工作的基础，也可以用于其他相关研究的参考。
